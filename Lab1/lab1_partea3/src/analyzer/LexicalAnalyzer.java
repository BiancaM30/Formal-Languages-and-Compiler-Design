...
//    public createFIP(List<Token> tokens) {
//
//    }
    ...
    public Map<String, Pair> createTSConstants(List<Token> tokens) {
        ...
        int nrConstants = 0;
        BSTNodeConst root = null;
        //creare arbore binar de cautare
        for (Token token : tokens) {
            if (token.getType().equals(TokenType.CONSTANT) && (!search(Integer.parseInt(token.getName()), root))) {
                int value = Integer.parseInt(token.getName());
                if (nrConstants == 0) root = new BSTNodeConst(value);
                else root = BSTNodeConst.insert(value, root);
                nrConstants++;
        ...
        for (Token token : tokenListInOrder) {
            if (root.levelOrder(root, Integer.parseInt(token.getName())) != null) {
                int left, right;
                BSTNodeConst node = root.levelOrder(root, Integer.parseInt(token.getName()));
    ...
    public List<Pair> createFIP(List<Token> tokens) {
        for (Token token : tokens) {
            ...
            if (token.getType() == TokenType.CONSTANT) {
                left = 1;
                right = getPositionFromTsConstants(Integer.parseInt(token.getName()));
    ...
    public int getPositionFromTsConstants(int elem) {
        int poz = 1;
        for (Map.Entry<String, Pair> entry : tsConstants.entrySet()) {
            if (Integer.parseInt(entry.getKey()) == elem) {
                return poz;
            }
            poz++;
        }
